...
exports = module.exports = function(mocker){
  ...
  router.get('/space/:id', function(req, res) {
    var result = mocker.getSpaceDetail(req.param('id'));
  ...
  router.post('/space/:id?', function(req, res) {
    var result;
    if(req.param('id')){
      result = mocker.updateSpace(req.body);
    }else{
      result = mocker.addSpace(req.param('path'), req.param('description'));
      mocker.addGroup(result.id, 'default group');
    }
    res.json(result);
  });
  ...
  router.post('/rule/:id?', function(req, res) {
    var result;
    if(req.param('id')){
      result = mocker.updateRule(req.body);
    }else{
      result = mocker.addRule(req.param('groupId'), req.param('name'), req.param('index'));
    }
    res.json(result);
  });
  ...
  router.post('/sort/rule', function(req, res){
    var result = mocker.sortGroup(req.param('groupId'), req.param('ruleIds'));
    res.json(result);
  });
