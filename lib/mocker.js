...
Mocker.prototype.refreshRouter = function(id){
  var self = this;
  var targetSpaces;
  ...
  targetSpaces.forEach(function(space){
    ...
    self.router.use('/' + space.path, subRouter);
    //mapping rules
    var detail = self.getSpaceDetail(space.id);
    if(detail && detail.rules) {
      detail.rules.forEach(function (rule) {
        var handler = Handler.get(rule);
        handler.name = rule.id;
        subRouter[rule.method]('/' + rule.path, handler);
      });
    }
  });
};
