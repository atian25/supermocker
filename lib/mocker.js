...
Mocker.prototype.updateSpace = function(attr){
  if(!attr || !attr.path){
    return this.errorHandler('updateSpace, illegal param: %j', attr);
  }else{
    ...
    if(space && (!attr.id || space.id != attr.id)){
      return this.errorHandler('space path is exist: %s at space: %j', attr.path, space);
    }else{
...
Mocker.prototype.sortGroup = function(spaceId, groupIds){
  ...
  if(!space){
    return this.errorHandler('space#%d not exist: %s ', spaceId);
  }else{
    var currentGroupIds = _.pluck(space.groups, 'id');
    if( _.xor(currentGroupIds, groupIds).length > 0){
      //not allow to change groupIds
      return this.errorHandler('space#%d\'s groupIds is not allow to change: %j -> %j', space.id, currentGroupIds, groupIds);
    }else{
...
Mocker.prototype.updateGroup = function(spaceId, attr){
  if(!attr || !attr.name){
    return this.errorHandler('updateGroup, illegal param: %j', attr);
  }else{
    var space = this.getSpace(spaceId);
    if(!space){
      return this.errorHandler('space#%d not exist: %s ', spaceId);
