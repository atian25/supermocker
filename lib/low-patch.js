...
var Low = module.exports = require('lowdb');

Low.mixin(require('underscore.db'));

Low.mixin({
  /**
   * Create an object with all falsey values removed. The values false, null, 0, "", undefined, and NaN are all falsey.
   * @param obj The object to compact
   * @returns {Object} Return a new object of filterd values.
   */
  compactObject: function(obj){
    return this.pick(obj, this.identity);
  },
  ...
  insertOrUpdate: function(collection, attrs, replace){
    ...
    if (doc){
      ...
      this.update(collection, id, attrs);
    }else{
  ...
  /**
   * reorder collection with the new order, will modify origin collection
   * @param {Array} collection
   * @param {Array} newOrderArr The new order, value is  id
   * @returns {Array} Return the collection
   */
  reorder: function(collection, newOrderArr){
    var self = this;
